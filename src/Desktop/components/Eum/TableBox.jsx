...
const TableBox = ({ tempData,setModalOn }) => {
  ...
  return (
      <div className={styles.tableContainer}>
        <div className={styles.menu} onClick={onModal}><span><img src="/images/menu-icon/config.png" alt="" /></span></div>
        <div id="table1" className={styles.tableWrapper}>
          <div className={styles.table}>
            <div className={styles.tableHeader}>
              <div className={styles.th} style={{ flex: "0 0 50px" }}></div>
              <div className={styles.th} style={{ flex: "0 0 150px" }}>
                ID
              </div>
            ...
            {tempData.map((item, i) => (
              <div className={styles.tableRow} key={i}>
                <div
                  className={styles.td}
                  style={{
                    backgroundColor: "#3162C7",
                    color: "white",
                    fontWeight: "400",
                    border: "1.5px solid #4e82ea",
                    flex: "0 0 50px",
                  }}
                >
                  {1 + i}
                </div>
                <div className={styles.td} style={{ flex: "0 0 150px" }}>
                  2024031{Math.floor(Math.random() * 100)}
                </div>
                ...
                <div
                  className={styles.td}
                >{`${Math.floor(Math.random() * 150) + 50}/${Math.floor(Math.random() * 100)}`}</div>
                <div className={styles.td}>102</div>
                <div
                  className={styles.td}
                >{`${Math.floor(Math.random() * 60)}M`}</div>
                <div
                  className={styles.td}
                >{`G${Math.floor(Math.random() * 5 + 1)}`}</div>
              </div>
            ))}
          </div>
        </div>
