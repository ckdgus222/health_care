...
<>
  ...
  <div className={`modal_container ${any}`}>
    <div className="modal_box">
      <h2>
        {viewType === "room" ? `Room ${data.left[0].roomNumber}` : `Patient ${data.NO}`}
      </h2>
      {viewType === "room" ? (
        <>
          <div className="modal_left">
            <h3>Left Column</h3>
            {data.left.map((item) => (
              <div key={`left-${item.NO}`} className="modal_item">
                <p>Name: {item.patName}</p>
                <p>Fluid: {item.fluid}</p>
                <p>Other Info: {item.otherInfo}</p>
              </div>
            ))}
          </div>
          <div className="modal_right">
            <h3>Right Column</h3>
            {data.right.map((item) => (
              <div key={`right-${item.NO}`} className="modal_item">
                <p>Name: {item.patName}</p>
                <p>Fluid: {item.fluid}</p>
                <p>Other Info: {item.otherInfo}</p>
              </div>
